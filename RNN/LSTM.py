...
#全部元素初始化为0
#将该张量复制给变量state
# 正确初始化LSTM的初始状态（元组h0, c0）
h0 = torch.zeros((1, batch_size, num_hiddens))
c0 = torch.zeros((1, batch_size, num_hiddens))
state = (h0, c0)  # LSTM需要元组状态
#挺高一个隐藏状态和一个输入，我们可以用更新后的隐藏状态计算输出
#创建一个形状为（num_steps,batch_size,len(vocab))的随机张量x
#用于表示输入的序列，每个时间步的输入为一个词汇表大小的独热编码向量
x=torch.rand(size=(num_steps,batch_size,len(vocab)))
#将输入x和初始隐藏状态state作为输入传递给循环神经网络层rnn_layer进行前向计算
#返回输出张量Y和更新后的隐藏专题state-new
y,state_new=lstm_layer(x,state)

print(y.shape, state_new[0].shape, state_new[1].shape)  # 输出LSTM输出和新状态的形状
#定义RNNModel类
